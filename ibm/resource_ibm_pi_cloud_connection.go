...
func resourceIBMPICloudConnectionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	cloudConnection, cloudConnectionJob, err := client.Create(body, cloudInstanceID)
	if err != nil {
	...
	if cloudConnection != nil {
		...
		_, err = waitForIBMPIJobCompleted(client, jobID, cloudInstanceID, d.Timeout(schema.TimeoutCreate))
		if err != nil {
			return fmt.Errorf(errors.CreateCloudConnectionOperationFailed, cloudInstanceID, err)
		}
	}
...
func resourceIBMPICloudConnectionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChanges(helpers.PICloudConnectionGlobalRouting,
		helpers.PICloudConnectionMetered, helpers.PICloudConnectionClassicEnabled,
		helpers.PICloudConnectionClassicGreCidr, helpers.PICloudConnectionClassicGreDest,
		helpers.PICloudConnectionVPCEnabled, helpers.PICloudConnectionVPCCRNs) {
		...
		_, cloudConnectionJob, err := client.Update(cloudConnectionID, cloudInstanceID, body)
		if err != nil {
			return fmt.Errorf(errors.UpdateCloudConnectionOperationFailed, cloudConnectionID, err)
		}
		if cloudConnectionJob != nil {
			_, err = waitForIBMPIJobCompleted(jobClient, *cloudConnectionJob.ID, cloudInstanceID, d.Timeout(schema.TimeoutCreate))
			if err != nil {
				return fmt.Errorf(errors.UpdateCloudConnectionOperationFailed, cloudConnectionID, err)
			}
		}
	...
	if d.HasChange(helpers.PICloudConnectionNetworks) {
		...
		for _, n := range expandStringList(toAdd.List()) {
			jobReference, err := client.AddNetwork(n, cloudConnectionID, cloudInstanceID)
			if err != nil {
				return fmt.Errorf("failed to perform the network add operation... %v", err)
			}
			_, err = waitForIBMPIJobCompleted(jobClient, *jobReference.ID, cloudInstanceID, d.Timeout(schema.TimeoutUpdate))
			if err != nil {
				return fmt.Errorf(errors.UpdateCloudConnectionOperationFailed, cloudConnectionID, err)
			}
		}
		...
		for _, n := range expandStringList(toRemove.List()) {
			jobReference, err := client.DeleteNetwork(n, cloudConnectionID, cloudInstanceID)
			if err != nil {
...
func resourceIBMPICloudConnectionRead(d *schema.ResourceData, meta interface{}) error {
	...
	client := st.NewIBMPICloudConnectionClient(sess, cloudInstanceID)
	cloudConnection, err := client.Get(cloudConnectionID, cloudInstanceID)
	if err != nil {
		switch err.(type) {
	...
	d.Set(helpers.PICloudConnectionId, cloudConnection.CloudConnectionID)
...
func resourceIBMPICloudConnectionDelete(d *schema.ResourceData, meta interface{}) error {
	...
	_, deleteJob, err := client.Delete(cloudConnectionID, cloudInstanceID)
	if err != nil {
	...
	if deleteJob != nil {
		...
		client := st.NewIBMPIJobClient(sess, cloudInstanceID)
		_, err = waitForIBMPIJobCompleted(client, jobID, cloudInstanceID, d.Timeout(schema.TimeoutDelete))
		if err != nil {
			return fmt.Errorf(errors.DeleteCloudConnectionOperationFailed, cloudConnectionID, err)
		}
	}
